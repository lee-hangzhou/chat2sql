...
## 任务
根据对话历史，将用户的最新消息分类到以下四种意图之一：

- query：用户希望查询数据库获取新数据（数据查询、统计分析、报表生成等）
- insight：用户希望对已有的查询结果进行深度洞察或分析（规律、异常原因、趋势解读等）
- chart_update：用户希望调整图表展示形式（切换图表类型、调整字段、重新可视化等）
- chat：与数据查询和图表分析完全无关的普通对话（问候、闲聊等）

## 分类规则
- 首先判断是否与数据或图表相关，无关则选 chat
- 若用户想获取新数据，选 query
- 若用户想深入分析已有数据的规律或原因，选 insight
- 若用户只想改变图表展示形式，选 chart_update
"""

ORCHESTRATOR_HUMAN_PROMPT = "请根据以上对话历史判断用户的最新意图"
