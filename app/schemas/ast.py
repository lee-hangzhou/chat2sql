...
class WindowElement(BaseModel):
    ...
    frame: Optional[str] = Field(
        default=None,
        description="窗口帧定义，例如 ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW",
    )


class CaseWhenElement(BaseModel):
    """CASE 表达式的单个 WHEN 分支"""
    when: FilterElement = Field(..., description="条件")
    then: str = Field(..., description="满足条件时的结果表达式")


class CaseElement(BaseModel):
    """CASE WHEN ... THEN ... ELSE ... END 表达式"""
    whens: List[CaseWhenElement] = Field(..., description="WHEN-THEN 分支列表")
    else_result: Optional[str] = Field(default=None, description="ELSE 分支结果表达式")
    alias: Optional[str] = Field(default=None, description="CASE 表达式别名")
...
class ColumnElement(BaseModel):
    ...
    case: Optional[CaseElement] = Field(default=None, description="CASE 表达式")
...
class UnionElement(BaseModel):
    """集合操作节点"""
    all: bool = Field(default=True, description="是否 UNION ALL")
    queries: List["QueryElement"] = Field(..., description="参与集合操作的查询列表")


class CTEElement(BaseModel):
    """公用表表达式（WITH 子句中的单个 CTE）"""
    name: str = Field(..., description="CTE 名称")
    query: "QueryElement" = Field(..., description="CTE 定义的查询")
...
class QueryElement(BaseModel):
    ...
    ctes: Optional[List[CTEElement]] = Field(default=None, description="公用表表达式列表（WITH 子句）")
