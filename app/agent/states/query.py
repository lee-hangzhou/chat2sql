...
from app.schemas.agent import SQLResult, ValidatedCandidate, CandidateExecResult, SyntaxResult, \
    PerformanceResult
...
class QueryState(BaseModel):
    ...
    validated_candidates: List[ValidatedCandidate] = Field(default_factory=list,
                                                           description="通过校验的候选，由 sql_validator 写入")
    candidate_exec_results: List[CandidateExecResult] = Field(default_factory=list,
                                                              description="已执行候选的比对结果，由 sql_selector 写入")
